...
To test the feasibility of our Lock-in-Pop design scheme described in
Section \ref{sec.design}, we used it to implement a secure virtual machine
dubbed Lind. An old English word for a shield constructed from two layers of
...
LOC, yet still protects vulnerable OS kernel code from exploitation by
untrusted user programs. The dual-sandbox design also minimizes overall overhead
...
As explained in Section \ref{sec.design}, the Lock-in-Pop design utilizes both a
computational module and a Library OS module. In Lind, NaCl serves as the former,
while Repy, and the POSIX interface built into it, perform the tasks of the latter.
...
\lois{I am not sure how to format it, but insert a subhead here 5.1 Primary
Components}
textbf{Native Client.}
...
\lois{I am not sure how to format it, but insert a subhead here "5.2 Enhanced
Safety in Call Handling}
...
In the Lock-in-pop design scheme, the file system API only need
to provide functionality of writing data to storage.% \yanyan{where is the example?}
