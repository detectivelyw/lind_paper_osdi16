...
Those additional kernel paths are added because some functions in Repy
have more capabilities for message sending and network connection than Lind's
 system call interface. For example, in Repy,
...
Since it is the direct point of connect with the OS kernel, it could be argued
that Repy sandbox kernel is the weakest link in terms of the overall security coverage provided by Lind. 
And SafePOSIX, built on Repy, could become a security weakness. 
...
because they run only in the NaCl computation sandbox. No system calls are
...
each repeatedly calls into SafePOSIX to re-create
the call. This additional SafePOSIX computation produced the additional
...
level that is barely noticeable to the users, even
 ...
 to the magnitude of 10000 ms ensures that
