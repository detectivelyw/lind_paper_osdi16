...
within its SafePOSIX module. 
\lois{What does Lind do if an app needs symbolic link}
\yiwen{This is a good question. But maybe it is not the right place to discuss it.} 
The bug would (at most) enable an attacker to execute
...
additional popular paths.\lois{The 5.8\% thing is confusing. If you are using this
...
\lois{But these paths are safe?} 
\yiwen{Yes, because they are popular paths.}
For example, in Repy,
...
that Repy sandbox kernel could be a weakness in the overall security coverage provided by Lind.
...
each repeatedly calls into SafePOSIX. 
\lois{Maybe explain why they have to repeatedly call in?} 
\yiwen{I/O heavy means I/O system calls are needed repeatedly. So I think this is fine.} 
This additional SafePOSIX computation produced the additional
...
1.4x to 2x 
\lois{It is confusing to mix percent signs for one side of the range and
magnitude x for the other}, 
\yiwen{I think we do not have percent signs now.}
when running applications such as Apache and UNIX benchmark.
