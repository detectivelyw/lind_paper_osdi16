...
\lois{Remember to change "commonly-used" kernel path to "popular" kernel paths in
the legend. Also the three figures 2 and three are to be identified as "Bug Distribution
by age of code" and "Bug distribution in drivers"}
...
performed a power analysis as follows. \lois{If we are making the comparison with
the other metric Section 3.3, then I would take out the "statistical significance"
subhead}
...
by this age-based metric. \lois{In the analysis, remember to include the argument
that a system can not be run exclusively on old code.}
...
only 10.0\% of the kernel bugs could be detected.\lois{remember to cite the newer Palix
study. Fewer driver bugs may be the result of a concentrated effort to improve}
