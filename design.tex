...
system calls, and a system functionality safe re-creation API that implements more
...
safer re-creation interface. One important design rule is to only rely on the
sandbox kernel API when trying to acquire hardware resources from the underlying OS kernel.
The safe re-creation technique will then use those basic resources, such as memory allocations,
